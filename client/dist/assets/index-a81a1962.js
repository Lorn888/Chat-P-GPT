...
function g(t, n) {
  let o = 0,
    s = setInterval(() => {
      o < n.length ? ((t.innerHTML += n.charAt(o)), o++) : clearInterval(s);
    }, 20);
}
function h() {
  const t = Date.now(),
...
const u = async (t) => {
  ...
  (a.innerHTML += d(!1, n.get("prompt"))), c.reset();
  const o = h();
  (a.innerHTML += d(!0, " ", o)), (a.scrollTop = a.scrollHeight);
  const s = document.getElementById(o);
  m(s);
  const e = await fetch("https://chat-p-back.onrender.com", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ prompt: n.get("prompt") }),
  });
  if ((clearInterval(l), (s.innerHTML = " "), e.ok)) {
    const i = (await e.json()).bot.trim();
    g(s, i);
  } else {
    const r = await e.text();
    (s.innerHTML = "Something went wrong"), alert(r);
  }
};
